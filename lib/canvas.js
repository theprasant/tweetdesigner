...
const drawTweetQuoteCanvas = async (width, height, { text, name, username, pfp, followers, followings, fontArr, returnFormat, returnType, returnQuality }) => {
  ...
  let fontSize;
  if (text.length <= 60) fontSize = 60;
  else if (text.length > 60 && text.length <= 100) fontSize = 50;
  else if (text.length > 100 && text.length <= 150) fontSize = 39;
  else if (text.length > 150 && text.length <= 180) fontSize = 35;
  else if (text.length > 180 && text.length <= 210) fontSize = 32;
  else if (text.length > 210 && text.length <= 250) fontSize = 30;
  else if (text.length > 250 && text.length <= 295) fontSize = 29;
  else if (text.length > 295 && text.length <= 310) fontSize = 28;
  else fontSize = 26;
  ...
  // shade
  const shade = ctx.createLinearGradient(0, height / 2, height, height / 2);
  ...
  // ctx.fillStyle = "red";
  ctx.font = "bold 57px Baloobhaijaan";
  ...
  ctx.fillText(`Created with @${config.username}`, 20, height - (waterMarkH/2));
...
//   text: `Lorem ipsum dolor sit amet consectetur adipisicing elit. Perferendis, tempore?`,
//   name: "Prasant Kumar",
//   username: 'decodeprasant',
//   pfp: 'https://cdn.discordapp.com/avatars/745688196440129915/b86d1c4279033b1232a4dd098566d116.png?size=2048',
//   followers: "2.3 M",
//   followings: "254",
