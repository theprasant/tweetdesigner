...
const drawTweetQuoteCanvas = async (width, height, { text, name, username, pfp, followers, followings, fontArr, returnFormat, returnType, returnQuality }) => {
  ...
  try {
    ...
    if (langsArr.find(lang => lang.language === 'ar')) {
      ...
      // await fillTextWithTwemoji(ctx, text, (height + width) / 2 - 50, txtY , width - height);
    } else {
  ...
  ctx.textAlign = "center";
  let nameFontSize = 47;
  ctx.font = `bold ${nameFontSize}px Baloobhaijaan`;
  ctx.fillStyle = "#ffffffe2"
  // ctx.fillText(name, (height + width) / 2 - 50, nameY, width - height)
  
  let nametextmeasure = ctx.measureText(name, width - height);
  let nameWidth = nametextmeasure.width;
  // console.log('nameFontSize: ', nameFontSize, 'nameWidth: ', nameWidth, 'width - height: ', width - height)
  while (nameWidth > width - height || nametextmeasure.lines.length > 1) {
    // ctx.font = "bold 40px Baloobhaijaan";
    nameFontSize -= 1;
    ctx.font = `${nameFontSize}px Baloobhaijaan`;
    nametextmeasure = ctx.measureText(name, width - height);
    nameWidth = nametextmeasure.width;
    // console.log('nameFontSize: ', nameFontSize, 'nameWidth: ', nameWidth, 'width - height: ', width - height)
    // console.log('font: ', ctx.font)
  }
  try {
    let nameLangArr = await detectlanguage.detect(name);
    if (nameLangArr.find(lang => lang.language === 'ar')) {
      ctx.direction = 'rtl';
    } else {
      ctx.direction = 'ltr';
    }
  ...
  ctx.fillText(`@${config.username} creation`, 20, height - (waterMarkH / 2));
