...
const replyMentionedTweets = async (mentionsArr, { statusText, statusTextFormatter }) => {
  ...
  for (let mention of mentionsArr) {
    ...
    try {
      ...
      let tweetQuoteImageBuffer = await drawTweetQuoteCanvas(800, 600, {
        text: tweet.full_text,
        username: tweet.user.screen_name,
        pfp: tweet.user.profile_image_url_https.replace('_normal', ''),
        bgImageURL: 'https://i.imgur.com/KJgefa9.png',
        fontArr: [path.join(__dirname, '../assets/fonts/Rajdhani/Rajdhani-Medium.ttf')],
        returnFormat: 'buffer',
...
const getOnlyRepliedMentions = async (mentionsArr, username) => {
  ...
  for (let mention of mentionsArr) {
    ...
    if (tweet.in_reply_to_status_id_str && tweet.text.includes(username) && tweet.user.screen_name != username) {
