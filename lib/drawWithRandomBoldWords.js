...
let countryFlagReg = /[🇦|🇧|🇨|🇩|🇪|🇫|🇬|🇭|🇮|🇯|🇰|🇱|🇲|🇳|🇴|🇵|🇶|🇷|🇸|🇹|🇺|🇻|🇼|🇽|🇾|🇿]+/g;
let emojiReg = /\p{Extended_Pictographic}/u;
...
// ctx.fillStyle = "#fff";
// ctx.font = '50px Arial';
// // await fillTextWithTwemoji(ctx, 'test 🇸🇦', 100, 100);
...
const drawWithRandomBoldWords = async (ctx, txt, startX, startY, args) => {
  // fillTextWithTwemoji(ctx, txt, 500, 100);
  ...
  console.log('startY after: ', startY)
  ...
  // lines.forEach((e, i) => {
    ...
    // words.forEach((word, j) => {
      ...
      if(countryFlagReg.test(word) || emojiReg.test(word)){
        ...
        await fillTextWithTwemoji(ctx, word, wordX, wordY);
      }else{

        ctx.fillText(word, wordX, wordY);
      }
      // console.log('font: ', ctx.font)
      // fillTextWithTwemoji(ctx, word, wordX, wordY);
